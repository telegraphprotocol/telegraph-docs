...
2. **Encode request** — Build parameters as `OnChainData` (see below). Endpoint and encoding depend on the subnet (see [Subnet inference API reference](#subnet-inference-api-reference) below).
...
### Subnet 19 — Nineteen (Completions, Image)

| Property | Value |
|----------|--------|
| **Subnet ID** | 19 |
| **Name** | Nineteen |
| **Purpose** | AI completions, image generation |

**HTTP**

| Item | Spec |
|------|------|
| **Completions** | POST `/19/chat/completions` or `/19/completions` — same shape as Chutes (model, messages or prompt, temperature, max_tokens). |
| **Image** | POST `/19/text-to-image` — body: `prompt`, `model` (e.g. stable-diffusion), optional `steps`, `width`, `height`. |
| **Response** | Completions: standard choices/usage; image: URL or base64 image. |

**Contract:** Encode prompt/model/options in `strings`/`integers`; decode text or image URL from `response.strings`.
...
| **Response** | Standard OpenAI-like: `model`, `choices` (e.g. `choices[0].message.content`), `usage` (prompt_tokens, completion_tokens, total_tokens) |
...
- [Dapp Examples — Interface & Implementation Guide](../dapp-examples/README.md) — Diamond interfaces and flows.
